...
function createMainWindow() {
  ...
  mainWindow.webContents.setWindowOpenHandler(({ url }) => {
    const devURL = process.env.VITE_DEV_SERVER_URL || "http://127.0.0.1:8080";
    const isAppUrl = url.startsWith("file://") || url.startsWith(devURL);
    if (isAppUrl) {
      return {
        action: "allow",
...
ipcMain.handle("license:get-info", async () => {
  try {
    ...
    if (!fs.existsSync(licensePath)) return { allowedPages: null };

    const raw = fs.readFileSync(licensePath, "utf8");
    const data = JSON.parse(raw || "{}");
    const allowedPages = Array.isArray(data?.allowedPages)
      ? data.allowedPages.map((p) => String(p || "").trim()).filter(Boolean)
      : null;
...
ipcMain.handle("print:current", async (event, options = {}) => {
  ...
  return new Promise((resolve) => {
    try {
      wc.print(
        { printBackground: true, silent: false, ...options },
...
app.whenReady().then(async () => {
  ...
  // In development, the backend is started by a separate dev script.
  // Only auto-start the bundled backend in production (unless explicitly disabled).
  if (!isDev && process.env.ELECTRON_NO_BACKEND !== "1") startBackend();
