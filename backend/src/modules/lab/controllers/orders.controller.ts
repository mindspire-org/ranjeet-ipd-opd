...
export async function create(req: Request, res: Response){
  ...
  // FBR: centralized posting via IMS/SDC SOAP service
  try {
    const subtotal = Number((doc as any)?.subtotal||0)
    const discount = Number((doc as any)?.discount||0)
    const net = Number((doc as any)?.net||0)
    const payload = { tokenNo, subtotal, discount, net }
    const r = await postFbrInvoiceViaSDC({ module: 'lab-order', invoiceType: 'LAB', refId: String((doc as any)?._id||''), amount: net, payload })
    if (r) {
      (doc as any)._doc = { ...(doc as any)._doc, fbr: { fbrInvoiceNo: r.fbrInvoiceNo, status: r.status, qrCode: r.qrCode, mode: r.mode, error: r.error } }
      if (r.status === 'SUCCESS' && r.mode === 'PRODUCTION'){
        try { await LabOrder.findByIdAndUpdate((doc as any)?._id, { $set: { isFbrLocked: true } }) } catch {}
      }
    }
  } catch {}
...
export async function updateTrack(req: Request, res: Response){
  ...
  // Prevent reversing/returns if Production FBR locked
  try {
    const existing: any = await LabOrder.findById(id).lean()
    if (!existing) return res.status(404).json({ message: 'Order not found' })
    if (existing.isFbrLocked && (patch as any).status === 'returned'){
      return res.status(409).json({ error: 'Order is locked by FBR (Production). Create a credit note or contact admin.' })
    }
  } catch {}
...
export async function remove(req: Request, res: Response){
  const { id } = req.params
  // Prevent delete when FBR locked (Production)
  try {
    const existing: any = await LabOrder.findById(id).lean()
    if (!existing) return res.status(404).json({ message: 'Order not found' })
    if (existing.isFbrLocked){
      return res.status(409).json({ error: 'Order is locked by FBR (Production). Deletion not allowed.' })
    }
  } catch {}
