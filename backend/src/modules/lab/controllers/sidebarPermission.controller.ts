...
const defaultSidebarItems = [
  ...
  { path: '/lab/user-management', label: 'User Management', order: 21 },
  { path: '/lab/audit-logs', label: 'Audit Logs', order: 22 },
  { path: '/lab/expenses', label: 'Expenses', order: 23 },
  { path: '/lab/pay-in-out', label: 'Pay In / Out', order: 24 },
  { path: '/lab/manager-cash-count', label: 'Manager Cash Count', order: 25 },
  { path: '/lab/settings', label: 'Settings', order: 26 },
]
...
const defaultVisibility = {
  ...
  pharmacist: defaultSidebarItems.map(item => ({
    ...item,
    visible: ['/lab/staff-management', '/lab/staff-settings', '/lab/staff-monthly', '/lab/user-management'].includes(item.path) ? false : true
  })),
  salesman: defaultSidebarItems.map(item => ({
    ...item,
    visible: ['/lab/inventory', '/lab/suppliers', '/lab/purchase-history', '/lab/staff-attendance', '/lab/staff-management', '/lab/staff-settings', '/lab/staff-monthly', '/lab/reports', '/lab/supplier-returns', '/lab/audit-logs', '/lab/expenses', '/lab/settings', '/lab/user-management'].includes(item.path) ? false : true
  }))
}
...
export async function updatePermissions(req: Request, res: Response) {
  ...
  // Log the action
  try {
    const { LabAuditLog } = await import('../models/AuditLog')
