...
export async function createOpd(req: Request, res: Response){
  ...
  const baseFee = hasOverride ? Math.max(0, Number(overrideFee)) : Math.max(0, resolvedFee)
  const discountInit = Math.max(0, Number((data as any).discount || 0))
  const finalFee = Math.max(0, baseFee - discountInit)
...
export async function update(req: Request, res: Response){
  ...
  const base = Number(tok.baseFee || 0) || (Number(tok.fee||0) + Number(tok.discount||0))
  const oldDiscount = Number(tok.discount||0)
  const oldFee = Number(tok.fee||0)
  const newFee = Math.max(0, base - newDiscount)
  tok.baseFee = base
  tok.discount = newDiscount
  tok.fee = newFee
  await tok.save()
  // Update encounter resolved fee
  try { if (tok.encounterId) await HospitalEncounter.findByIdAndUpdate(tok.encounterId, { $set: { consultationFeeResolved: newFee } }) } catch {}
  // Finance: reverse and re-post
  try { await reverseJournalByRef('opd_token', id, 'Discount updated') } catch (e) { console.warn('Finance reversal failed on token update', e) }
  try {
    await postOpdTokenJournal({
      tokenId: id,
      dateIso: String(tok.dateIso || new Date().toISOString().slice(0,10)),
      fee: newFee,
      doctorId: tok.doctorId || undefined,
      departmentId: tok.departmentId,
      patientId: String(tok.patientId || ''),
      patientName: String(tok.patientName || ''),
      mrn: String(tok.mrn || ''),
      tokenNo: String(tok.tokenNo || ''),
    })
  } catch (e) {
    console.warn('Finance posting failed for OPD token update', e)
  }
  // Audit
  try {
    const actor = (req as any).user?.name || (req as any).user?.email || 'system'
    await HospitalAuditLog.create({
      ...
      path: req.originalUrl,
      at: new Date().toISOString(),
      detail: `Token #${tok.tokenNo || id} — discount ${oldDiscount} → ${newDiscount}, fee ${oldFee} → ${newFee}`,
    })
  } catch {}
  res.json({ token: tok })
}
