...
export async function create(req: Request, res: Response) {
  ...
  // FBR: Auto-fiscalize invoice via IMS/SDC
  let fbrData: any = null
  try {
    const payload = {
      billNo: billNo,
      customer: data.customer || 'Walk-in',
      ...
      subtotal: subtotal,
      discount: discount,
      items: data.lines.map((l: any) => ({ name: l.name, qty: l.qty, price: l.unitPrice }))
    }

    const fbrResult = await postFbrInvoiceViaSDC({
      module: 'pharmacy',
    ...
    if (fbrResult) {
      fbrData = {
        fbrInvoiceNo: fbrResult.fbrInvoiceNo,
        status: fbrResult.status,
        qrCode: fbrResult.qrCode,
        mode: fbrResult.mode
      }

      // Lock invoice if successfully fiscalized in production
      if (fbrResult.status === 'SUCCESS' && fbrResult.mode === 'PRODUCTION') {
        try {
          await Dispense.findByIdAndUpdate(doc._id, { $set: { isFbrLocked: true } })
        } catch { }
      }
    }
  } catch (fbrError: any) {
    console.error('[FBR] Pharmacy fiscalization error:', fbrError.message)
  }
  ...
  // Attach FBR data to response
  const response: any = doc.toObject()
  if (fbrData) {
    response.fbr = fbrData
  }
