...
export async function summary(req: Request, res: Response){
  ...
  const expiringSoonCount = allItems.reduce((s:any,it:any)=>{
    ...
    if (!d) return s
    const dt = d.getTime()
    return s + ((dt >= today.getTime() && dt <= soon.getTime()) ? 1 : 0)
  }, 0)
  ...
  const expiringSoonItems = allItems
    .filter((it:any)=>{ const d = parseDate(it.earliestExpiry); return !!d && d.getTime() >= today.getTime() && d.getTime() <= soon.getTime() })
    .sort((a:any,b:any)=>{
