...
export async function create(req: Request, res: Response){
  ...
  if (data.type === 'Customer'){
    ...
    // Prevent returns on Production FBR-locked invoices
    if ((sale as any).isFbrLocked){
      throw new ApiError(409, 'Sale is locked by FBR (Production). Create a credit note or contact admin.')
    }
