...
export default function Doctor_Prescription() {
  ...
  async function searchMedicines(q: string) {
    try {
      ...
      const list: string[] = Array.isArray(res?.medicines)
        ? res.medicines
            .map((m: any) => String(m.name || m.genericName || "").trim())
            .filter(Boolean)
        : Array.isArray(res)
          ? res
              .map((m: any) =>
                String(m.name || m.genericName || m || "").trim(),
              )
              .filter(Boolean)
          : [];
  ...
  const save = async (e: React.FormEvent) => {
    ...
    if (!doc || !sel || !sel.encounterId) {
      showModal("Error", "Select a patient first", "error");
      return;
    }
  ...
  const sugLabTests = useMemo(
    () => Array.from(new Set([...labSeeds, ...loadList("labTest")])),
    [doc?.id, sugVersion],
  );
  ...
  async function printReferral() {
    try {
      ...
      if (!d) {
        showModal("Error", "Referral form not ready", "error");
        return;
      }
      ...
      showModal(
        "Error",
        e?.message || "Failed to open referral preview",
        "error",
      );
    }
  }
