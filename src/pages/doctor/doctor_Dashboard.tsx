...
export default function Doctor_Dashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const arr: Notification[] = (res?.notifications || []).map(
          (n: any) => ({
            id: String(n._id),
  ...
  async function load() {
    ...
    try {
      ...
      const [tokResRange, presRes] = await Promise.all([
      ...
      const presIds: string[] = (presRes?.prescriptions || []).map((r: any) =>
        String(r.encounterId?._id || r.encounterId || ""),
      );
      ...
      const myQueuedFiltered = myRange.filter((t) => {
        const encId = String(
          (t as any).encounterId?._id || (t as any).encounterId || "",
        );
      ...
      const presRows: PresRow[] = (presRes?.prescriptions || []).map(
        (r: any) => ({
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-slate-200 bg-gradient-to-r from-sky-100 to-violet-100 p-5">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <Link to="/doctor/prescription" className="btn">
            + New Prescription
          </Link>
          <Link to="/doctor/prescriptions" className="btn-outline-navy">
            Prescription History
          </Link>
          <Link to="/doctor/patients" className="btn-outline-navy">
            My Patients
          </Link>
        </div>
      </div>
...
function Stat({
  ...
  const tones: any = {
    ...
    emerald: "bg-emerald-100 text-emerald-700 border-emerald-100",
  };
  return (
