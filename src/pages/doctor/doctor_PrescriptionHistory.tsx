...
export default function Doctor_PrescriptionHistory() {
  ...
  const [confirmDlg, setConfirmDlg] = useState<{ open: boolean; target?: Prescription | null }>({ open: false })
  ...
  async function confirmDelete(){
    ...
    if (!confirmDlg.open || !p) { setConfirmDlg({ open: false }); return }
          ...
          {mine.map(p => (
            <div key={p.id} className="px-4 py-3 text-sm">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                <button className="rounded-md border border-rose-300 bg-rose-50 px-3 py-1 text-sm text-rose-700" onClick={()=>openDeleteConfirm(p)}>Delete</button>
              </div>
            </div>
          ))}
      ...
      {confirmDlg.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-sm rounded-xl bg-white p-4">
            <div className="mb-3 text-lg font-semibold text-slate-800">Delete Prescription</div>
