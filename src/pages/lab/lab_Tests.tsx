...
export default function Lab_Tests() {
  ...
  const performDelete = async () => {
    ...
    try {
      ...
      console.error(e);
      setNotice({ text: "Failed to delete test", kind: "error" });
    } finally {
      setDeleteOpen(false);
      setDeleteId(null);
      try {
  ...
  const exportCSV = async () => {
    ...
    const data = await loadAllForExport()
      ...
      .catch(() => pageItems);
    const header = ["Name", "Parameter", "Unit", "Price", "Created At"]
      .map(escape)
      .join(",");
    const lines = data
      .map((t) =>
  ...
  const exportPDF = async () => {
    ...
    for (const t of dataRows) {
      const data = [
        ...
        t.parameter || "",
        t.unit || "",
        `Rs ${(t.price ?? 0).toFixed(2)}`,
        new Date(t.createdAt).toLocaleString(),
      ];
      const lines = data.map((v, i) =>
        (doc as any).splitTextToSize(v, cols[i].width - 2),
      ) as string[][];
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-md bg-violet-700 px-3 py-2 text-sm font-medium text-white hover:bg-violet-800"
            >
          ...
          >
            ...
            <div className="mt-3 grid grid-cols-2 gap-3 text-sm text-slate-700">
              <div className="inline-flex items-center gap-2">
                <DollarSign className="h-4 w-4" /> {formatPKR(t.price)}
              </div>
            </div>
            ...
            <div className="mt-3 flex gap-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-md bg-rose-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-rose-700"
              >
      ...
      <div className="flex items-center justify-between text-sm text-slate-600 dark:text-slate-400">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="rounded-md border border-slate-300 p-1 disabled:opacity-40"
          >
          ...
          <button
            ...
            className="rounded-md border border-slate-300 p-1 disabled:opacity-40 dark:border-slate-600"
          >
      ...
      {deleteOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-xl bg-white shadow-2xl ring-1 ring-black/5 dark:bg-slate-900 dark:ring-white/10">
            <div className="border-b border-slate-200 px-5 py-3 text-base font-semibold text-slate-800 dark:border-slate-800 dark:text-slate-100">
              Confirm Delete
            </div>
