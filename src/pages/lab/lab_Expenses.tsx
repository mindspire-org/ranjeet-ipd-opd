...
export default function Lab_Expenses() {
  ...
  const [etype, setEtype] = useState<"All Types" | LabExpense["type"]>(
    "All Types",
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const res = await labApi.listExpenses({
          from: from || undefined,
          to: to || undefined,
        ...
        const list: LabExpense[] = (res.items || []).map((x: any) => ({
          id: x._id,
          date: x.date,
          type: x.type,
          note: x.note || "",
  ...
  const handleSave = async (exp: {
    date: string;
    type: LabExpense["type"];
    note: string;
    amount: number;
  }) => {
    try {
      await labApi.createExpense(exp);
    } catch (e) {
      console.error(e);
    }
    setTick((t) => t + 1);
  ...
  const performDelete = async () => {
    ...
    try {
      await labApi.deleteExpense(id);
      setNotice({ text: "Expense deleted", kind: "success" });
    } catch (e) {
      console.error(e);
      setNotice({ text: "Failed to delete expense", kind: "error" });
    } finally {
      setDeleteOpen(false);
      setDeleteId(null);
      setTick((t) => t + 1);
  ...
  const loadAllForExport = async (): Promise<LabExpense[]> => {
    ...
    while (true) {
      try {
        ...
        const mapped: LabExpense[] = items.map((x: any) => ({
          id: x._id,
          date: x.date,
          type: x.type,
          note: x.note || "",
  ...
  const exportCSV = async () => {
    ...
    const data = await loadAllForExport()
      .then((r) => (r.length ? r : filtered))
      .catch(() => filtered);
    const header = ["Date", "Type", "Note", "Amount"].map(escape).join(",");
    const lines = data
      .map((e) =>
        [e.date, e.type, e.note, (e.amount ?? 0).toFixed(2)]
          .map(escape)
          .join(","),
      )
      .join("\n");
  ...
  const exportPDF = async () => {
    ...
    doc.setFontSize(8);
    const dataRows: LabExpense[] = await loadAllForExport()
      .then((r) => (r.length ? r : filtered))
      .catch(() => filtered);
          ...
          <div>
            {total > 0 ? (
              <>
                ...
                {Math.min((page - 1) * limit + filtered.length, total)} of{" "}
                {total}
              </>
            ) : (
              "No results"
