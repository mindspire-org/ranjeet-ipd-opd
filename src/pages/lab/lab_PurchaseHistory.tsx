...
export default function Lab_PurchaseHistory() {
  ...
  const [viewer, setViewer] = useState<{ open: boolean; purchase: any | null }>(
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setLoading(true);
      try {
        const res = await labApi.listPurchases({
          from: from || undefined,
          to: to || undefined,
        ...
        const mapped: PurchaseSummary[] = items.map((p: any) => {
          ...
          const totalQty = lines.reduce(
            (s: number, l: any) =>
              ...
              Number(
                l.totalItems ??
                  Number(l.unitsPerPack || 1) * Number(l.packs || 0),
              ),
            0,
          );
        ...
        if (mounted && my === reqSeq.current) setLoading(false);
      }
    })();
    return () => {
      mounted = false;
  ...
  const requestDelete = (row: PurchaseSummary) => {
    setDeleteRow(row);
    setDeleteOpen(true);
  };
  const performDelete = async () => {
    ...
    try {
      ...
      console.error(e);
      setNotice({ text: "Failed to delete purchase", kind: "error" });
    } finally {
      setDeleteOpen(false);
      setDeleteRow(null);
      try {
  ...
  return (
    <div className="space-y-4">
      <div className="text-xl font-bold">Purchase History</div>
      {notice && (
        <div
                ...
                <tr key={r.id} className="hover:bg-slate-50/50 ">
                  ...
                  <td className="px-4 py-2">
                    <div className="flex gap-2">
                      <button
                        onClick={() =>
                          setViewer({ open: true, purchase: r.raw })
                        }
        ...
        <div className="flex items-center justify-between border-t border-slate-200 px-4 py-3 text-sm text-slate-600 ">
          <div>
            {total > 0 ? (
              <>
                ...
                {Math.min((page - 1) * limit + rows.length, total)} of {total}
              </>
            ) : (
              "No results"
            )}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={page <= 1}
              className="rounded-md border border-slate-200 px-2 py-1 disabled:opacity-40 hover:bg-slate-50 "
            >
              Prev
            </button>
            <div>
      ...
      {viewer.open && viewer.purchase && (
        <LinesDialog
          purchase={viewer.purchase}
          onClose={() => setViewer({ open: false, purchase: null })}
        />
      )}
      {deleteOpen && (
...
function LinesDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-3xl max-h-[90vh] flex flex-col rounded-xl bg-white p-0 shadow-2xl ring-1 ring-black/5 dark:bg-slate-900 dark:ring-white/10">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4 dark:border-slate-800">
          <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">
            Purchase {String(purchase?.invoice || "")}
          </h3>
          <button
            onClick={onClose}
            aria-label="Close"
            className="rounded-md p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-700 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-slate-100"
          >
            ×
          </button>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-5 text-sm text-slate-800 dark:text-slate-200">
            ...
            <table className="min-w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {lines.map((l: any, idx: number) => {
                  ...
                  const qty = Number(
                    l.totalItems != null ? l.totalItems : unitsPerPack * packs,
                  );
                  const buyPerPack = Number(l.buyPerPack || 0);
                  const buyPerUnit = Number(
                    l.buyPerUnit != null
                      ? l.buyPerUnit
                      : unitsPerPack
                        ? buyPerPack / unitsPerPack
                        : 0,
                  );
                  const amount =
                    buyPerPack > 0 ? buyPerPack * packs : buyPerUnit * qty;
                  return (
                    <tr key={idx}>
