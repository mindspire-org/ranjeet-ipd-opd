...
import Lab_CashMovementSlipDialog, {
  type CashMovementEntry,
} from "../../components/lab/lab_CashMovementSlipDialog";
...
export default function Pharmacy_PayInOut() {
  ...
  async function fetchMovements(p = page, l = limit) {
    setLoading(true);
    try {
      const res = await labApi.listCashMovements({
        from: from || undefined,
        to: to || undefined,
        search: search || undefined,
        type: typeFilter === "Any" ? undefined : typeFilter,
        page: p,
        limit: l,
      });
  ...
  async function fetchSummary() {
    try {
      const s = await labApi.cashMovementSummary({
        from: from || undefined,
        to: to || undefined,
        type: typeFilter === "Any" ? undefined : typeFilter,
      });
  ...
  const add = async () => {
    ...
    try {
      ...
      setNotice({ text: `Failed to save: ${msg}`, kind: "error" });
      try {
  ...
  const print = (e: CashMovementEntry) => {
    setSlipEntry(e);
    setOpenSlip(true);
  };
  ...
  function exportCSV() {
    ...
    a.href = url;
    a.download = `lab-cash-movements.csv`;
    a.click();
  ...
  return (
    <div className="w-full p-3 sm:p-4">
      <div className="text-xl font-semibold">Pay In / Pay Out</div>
      {notice && (
        <div
      ...
      <div className="mt-4 rounded-xl border border-slate-200 bg-white p-3 sm:p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            />
            <input
              placeholder="date/type/category/receiver/handover/note/user"
              value={search}
            ...
            <tfoot>
              <tr className="border-t border-slate-200 bg-slate-50 text-xs font-medium text-slate-700">
                <td className="px-3 py-2" colSpan={10}>
                  Totals — IN: PKR {totals.IN.toFixed(2)} | OUT: PKR{" "}
                  {totals.OUT.toFixed(2)} | NET: PKR {totals.NET.toFixed(2)}
                </td>
              </tr>
            </tfoot>
          </table>
