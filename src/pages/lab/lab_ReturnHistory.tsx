...
export default function Lab_ReturnHistory() {
  ...
  const [undoCtx, setUndoCtx] = useState<{
    id: string;
    reference: string;
    testId?: string;
    testName?: string;
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res: any = await labApi.listReturns({
        ...
        onConfirm={async (note) => {
          ...
          try {
            await labApi.undoReturn({
              reference: undoCtx.reference,
              testId: undoCtx.testId,
              testName: undoCtx.testName,
              note: note || undefined,
            });
            ...
            try {
              window.dispatchEvent(
                new CustomEvent("lab:return", {
                  detail: { reference: undoCtx.reference, undo: true },
                }),
              );
            } catch {}
          } catch (e) {
            console.error(e);
            setNotice({ text: "Failed to undo return", kind: "error" });
            try {
