...
import Lab_CashCountSlipDialog, {
  type CashCountEntry,
} from "../../components/lab/lab_CashCountSlipDialog";
...
export default function Pharmacy_ManagerCashCount() {
  ...
  const [summary, setSummary] = useState<{ amount: number; count: number }>({
    amount: 0,
    count: 0,
  });
  ...
  async function fetchCounts(p = page, l = limit) {
    setLoading(true);
    try {
      const res = await labApi.listCashCounts({
        from: from || undefined,
        to: to || undefined,
  ...
  useEffect(() => {
    fetchCounts(page, limit);
  }, [page, limit]);

  async function fetchSummary() {
    try {
      const s = await labApi.cashCountSummary({
        from: from || undefined,
        to: to || undefined,
        search: search || undefined,
      });
  ...
  const add = async () => {
    ...
    if (!isFinite(amt) || amt <= 0) {
      setNotice({ text: "Enter a valid amount", kind: "error" });
      try {
    ...
    try {
      ...
      setNotice({ text: "Failed to save", kind: "error" });
      try {
  ...
  const amountOf = (e: CashCountEntry) => {
    const a =
      typeof e.amount === "number" && isFinite(e.amount) ? Number(e.amount) : 0;
    if (a > 0) return a;
    return Object.entries(e.counts || {}).reduce(
      (s, [den, qty]) => s + Number(den) * Number(qty || 0),
      0,
    );
  };
  const print = (e: CashCountEntry) => {
    setSlipEntry(e);
    setOpenSlip(true);
  };
  ...
  function exportCSV() {
    const rows = [
      ["Date", "Amount", "Receiver", "Handover By", "Note", "User", "Entry ID"],
      ...list.map((e) => [
        e.date,
        String(amountOf(e)),
        e.receiver || "",
    ...
    a.href = url;
    a.download = `lab-cash-counts.csv`;
    a.click();
  ...
  return (
    <div className="w-full p-3 sm:p-4">
      <div className="text-xl font-semibold ">Manager Cash Count</div>
      {notice && (
        <div
          className={`mt-3 rounded-md border px-3 py-2 text-sm ${notice.kind === "success" ? "border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-900/40 dark:bg-emerald-900/20 dark:text-emerald-200" : "border-rose-200 bg-rose-50 text-rose-800 dark:border-rose-900/40 dark:bg-rose-900/20 dark:text-rose-200"}`}
        >
          {notice.text}
        </div>
      )}
        ...
        <div className="grid gap-2 sm:grid-cols-6">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Manager handing over"
              className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 "
            />
          </div>
      ...
      <div className="mt-4 rounded-xl border border-slate-200 bg-white p-3 sm:p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            />
            <input
              placeholder="date, amount, receiver, handover, note, user"
              value={search}
            ...
            <select
              ...
              className="rounded-md border border-slate-300 bg-white px-2 py-2 text-xs text-slate-900"
            >
            ...
            <tfoot>
              <tr className="border-t border-slate-200 bg-slate-50 text-xs font-medium text-slate-700">
                <td className="px-3 py-2" colSpan={7}>
                  Page Total — PKR{" "}
                  {filtered.reduce((s, e) => s + amountOf(e), 0).toFixed(2)} |
                  Grand Total — PKR {summary.amount.toFixed(2)} (Entries:{" "}
                  {summary.count})
                </td>
              </tr>
            </tfoot>
