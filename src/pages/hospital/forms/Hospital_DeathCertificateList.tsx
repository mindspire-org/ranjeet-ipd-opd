...
export default function Hospital_DeathCertificateList() {
  ...
  async function load() {
    ...
    try {
      const res: any = await hospitalApi
        .listIpdDeathCertificates({ q, page, limit })
        .catch(() => null);
      ...
      const mapped = await Promise.all(
        admissions.map(async (e: any) => {
          try {
            const dc: any = await hospitalApi
              .getIpdDeathCertificate(String(e._id))
              .catch(() => null);
            if (dc?.certificate) {
              return {
  ...
  async function onPrint(encounterId: string) {
    return openHtmlDoc(
      "Death Certificate",
      `/hospital/ipd/admissions/${encodeURIComponent(encounterId)}/death-certificate/print`,
    );
  }
  ...
  async function onDownloadPdf(encounterId: string) {
    ...
    return openPdfDoc("Death Certificate (PDF)", url);
  }

  async function doDelete(encounterId: string) {
    try {
      await hospitalApi.deleteIpdDeathCertificate(encounterId);
    } catch (e: any) {
                ...
                <td className="px-3 py-2">
                  <div className="flex gap-2">
                    <button
                      ...
                      onClick={() =>
                        navigate(
                          `/hospital/ipd/admissions/${encodeURIComponent(r.encounterId)}/forms/death-certificate`,
                        )
                      }
                    >
                      Edit
