...
export default function Hospital_DischargeSummaryList() {
  ...
  async function load() {
    ...
    try {
      const res: any = await hospitalApi
        .listIpdDischargeSummaries({ q, page, limit })
        .catch(() => null);
      ...
      const mapped = await Promise.all(
        admissions.map(async (e: any) => {
          try {
            const ds: any = await hospitalApi
              .getIpdDischargeSummary(String(e._id))
              .catch(() => null);
            if (ds?.summary) {
              return {
                _id: ds.summary._id,
  ...
  async function onPrint(encounterId: string) {
    return openHtmlDoc(
      "Discharge Summary",
      `/hospital/ipd/admissions/${encodeURIComponent(encounterId)}/discharge-summary/print`,
    );
  }
  ...
  async function onDownloadPdf(encounterId: string) {
    ...
    return openPdfDoc("Discharge Summary (PDF)", url);
  }

  async function doDelete(encounterId: string) {
    try {
      await hospitalApi.deleteIpdDischargeSummary(encounterId);
    } catch (e: any) {
                ...
                <td className="px-3 py-2">
                  <div className="flex gap-2">
                    <button
                      ...
                      onClick={() =>
                        navigate(
                          `/hospital/ipd/admissions/${encodeURIComponent(r.encounterId)}/forms/discharge-summary`,
                        )
                      }
                    >
                      Edit
