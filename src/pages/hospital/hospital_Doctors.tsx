...
import Hospital_AddDoctorDialog, {
  type HospitalDoctorInput,
} from "../../components/hospital/Hospital_AddDoctorDialog";
...
export default function Hospital_Doctors() {
  ...
  async function reload() {
    try {
      ...
      const depArray: any[] = ((depRes as any)?.departments ||
        (depRes as any) ||
        []) as any[];
      const deps: Array<{ id: string; name: string }> = depArray.map(
        (x: any) => ({ id: String(x._id || x.id), name: String(x.name || "") }),
      );
      setDepartments(deps);
      const items = (docRes as any).doctors.map((d: any) => {
        const depName = d.primaryDepartmentId
          ? deps.find(
              (z: { id: string; name: string }) =>
                z.id === String(d.primaryDepartmentId),
            )?.name || ""
          : "";
        return {
          id: d._id,
      ...
      showModal("Error", msg || "Failed to save doctor", "error");
    }
  }
  ...
  const addDoctor = async (addForm: HospitalDoctorInput) => {
    ...
    try {
      ...
      showModal("Error", msg || "Failed to add doctor", "error");
    }
  };
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="w-56 rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-200"
          />
          <button
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <input
                  value={editForm.cnic}
                  onChange={(e) =>
                    setEditForm((f) => ({ ...f, cnic: e.target.value }))
                  }
                  className="w-full rounded-md border border-slate-300 px-3 py-2 outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-200"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditForm((f) => ({ ...f, shares: e.target.value }))
                  }
                  className="w-full rounded-md border border-slate-300 px-3 py-2 outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-200"
                />
              </div>
