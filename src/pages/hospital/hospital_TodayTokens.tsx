...
export default function Hospital_TodayTokens() {
  ...
  const [editState, setEditState] = useState<{ row: TokenRow; value: string } | null>(null)
  ...
  async function load(){
    ...
    const items: TokenRow[] = (res.tokens || []).map((t: any) => ({
      ...
      baseFee: Number(t.baseFee ?? (Number(t.fee||0) + Number(t.discount||0) || 0)),
  ...
  return (
    <div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-3">
        ...
        <StatCard title="Discounted Tokens" value={discountedCount} tone="green" />
              ...
              <tr key={r._id} className={`text-slate-700 ${r.status === 'returned' ? 'bg-amber-50' : ''}`}>
                ...
                <Td>
                  <div className="flex gap-2">
                    <button disabled={actioningId===r._id} onClick={()=>setEditState({ row: r, value: String(r.discount||0) })} title="Edit" className="text-blue-600 hover:text-blue-800 disabled:opacity-50">✏️</button>
      ...
      {editState && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-sm rounded-lg bg-white p-4 shadow-lg">
            <div className="text-sm font-semibold text-slate-800">Edit Discount</div>
            <div className="mt-2 text-xs text-slate-600">Token #{editState.row.tokenNo} • Base Fee: Rs. {editState.row.baseFee.toLocaleString()}</div>
            <div className="mt-3">
              <label className="mb-1 block text-sm text-slate-700">Discount (PKR)</label>
              <input
            ...
            <div className="mt-4 flex justify-end gap-2 text-sm">
              <button onClick={()=> setEditState(null)} className="rounded-md border border-slate-300 px-3 py-1.5 text-slate-700 hover:bg-slate-50">Cancel</button>
              <button
                onClick={async ()=>{
                  const n = Number(editState.value)
                  if (!Number.isFinite(n) || n < 0){ alert('Invalid discount'); return }
                  if (n > editState.row.baseFee){ alert('Discount cannot exceed base fee'); return }
                  try {
                    setActioningId(editState.row._id)
                    await hospitalApi.updateToken(editState.row._id, { discount: n })
                    await load()
                    setEditState(null)
                  } catch (e: any){
