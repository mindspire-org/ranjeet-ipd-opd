  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (
        department !== "All" &&
        r.department !==
          (departments.find((x) => x._id === department)?.name || r.department)
      )
        return false;
      if (
        doctor !== "All" &&
        r.doctor !== (doctors.find((x) => x._id === doctor)?.name || r.doctor)
      )
        return false;
      if (!q) return true;
      return [
    ...
    <div>
        ...
        <div className="w-full lg:w-auto">
          <div className="grid grid-cols-2 gap-2 text-sm sm:flex sm:flex-wrap sm:items-center">
            ...
            <select
              ...
              className="col-span-2 rounded-md border border-slate-300 px-2 py-1 sm:col-span-1"
            >
            ...
            <button className="col-span-2 rounded-md border border-slate-300 px-3 py-1.5 hover:bg-slate-50 sm:col-span-1">
              Export CSV
            </button>
          </div>
        </div>
          ...
          <table className="min-w-[980px] w-full divide-y divide-slate-200 text-sm">
            <thead className="bg-slate-50">
              <tr className="text-left text-slate-600">
                ...
                <Th className="hidden xl:table-cell">CNIC</Th>
            ...
            <tbody className="divide-y divide-slate-200">
              {pageRows.map((r, idx) => (
                <tr key={idx} className="text-slate-700">
                  <Td>{r.date}</Td>
                  <Td className="hidden sm:table-cell">{r.time}</Td>
                  <Td>{r.tokenNo}</Td>
                  <Td>{r.mrNo}</Td>
                  <Td className="font-medium">{r.patient}</Td>
                  <Td className="hidden lg:table-cell">
                    {r.guardianRel
                      ? `${r.guardianRel} ${r.guardianName || ""}`
                      : "-"}
                  </Td>
                  <Td className="hidden xl:table-cell">{r.cnic || "-"}</Td>
                  <Td className="hidden md:table-cell">{r.age}</Td>
                  <Td className="hidden md:table-cell">{r.gender}</Td>
                  <Td className="hidden md:table-cell">{r.phone}</Td>
                  <Td className="hidden sm:table-cell">{r.doctor}</Td>
                  <Td className="hidden lg:table-cell">{r.department}</Td>
      ...
      {showSlip && slipData && (
        <Hospital_TokenSlip
          ...
          data={slipData as TokenSlipData}
          autoPrint={false}
        />
      )}
    </div>
