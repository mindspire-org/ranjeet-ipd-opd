...
export default function Hospital_DoctorSchedules(){
  ...
  useEffect(()=>{ if(doctorId && dateIso) load() }, [doctorId, dateIso])

  async function load(){
    setLoading(true)
    try{
      const res = await hospitalApi.listDoctorSchedules({ doctorId, date: dateIso }) as any
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <form onSubmit={save} className="grid grid-cols-1 gap-3 md:grid-cols-6">
          ...
          <div>
            ...
            <select value={form.departmentId} onChange={e=>setForm(f=>({ ...f, departmentId: e.target.value }))} className="w-full rounded-md border border-slate-300 px-3 py-2">
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 text-sm">
          <div className="font-medium text-slate-800">Schedules on {dateIso || '-'}</div>
