...
import Pharmacy_CashMovementSlipDialog, {
  type CashMovementEntry,
} from "../../components/pharmacy/pharmacy_CashMovementSlipDialog";
...
export default function Pharmacy_PayInOut() {
  ...
  async function fetchMovements(p = page, l = limit) {
    setLoading(true);
    try {
      const res = await pharmacyApi.listCashMovements({
        from: from || undefined,
        to: to || undefined,
  ...
  async function fetchSummary() {
    try {
      const s = await pharmacyApi.cashMovementSummary({
        from: from || undefined,
        to: to || undefined,
      });
  ...
  function exportCSV() {
    ...
    a.href = url;
    a.download = `pharmacy-cash-movements.csv`;
    a.click();
  ...
  return (
    <div className="w-full p-3 sm:p-4">
      ...
      <Pharmacy_CashMovementSlipDialog
        open={openSlip}
