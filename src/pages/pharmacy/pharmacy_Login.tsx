...
export default function Pharmacy_Login() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setError('')
    try {
      const res: any = await pharmacyApi.login(username.trim(), password)
      try {
        localStorage.setItem('token', res.token)
        localStorage.setItem('pharmacy.token', res.token)
        localStorage.setItem('pharmacy.user', JSON.stringify(res.user))
