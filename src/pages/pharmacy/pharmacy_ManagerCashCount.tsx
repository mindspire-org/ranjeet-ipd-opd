...
import Pharmacy_CashCountSlipDialog, {
  type CashCountEntry,
} from "../../components/pharmacy/pharmacy_CashCountSlipDialog";
...
export default function Pharmacy_ManagerCashCount() {
  ...
  async function fetchCounts(p = page, l = limit) {
    setLoading(true);
    try {
      const res = await pharmacyApi.listCashCounts({
        from: from || undefined,
        to: to || undefined,
  ...
  async function fetchSummary() {
    try {
      const s = await pharmacyApi.cashCountSummary({
        from: from || undefined,
        to: to || undefined,
        search: search || undefined,
      });
  ...
  function exportCSV() {
    ...
    a.href = url;
    a.download = `pharmacy-cash-counts.csv`;
    a.click();
