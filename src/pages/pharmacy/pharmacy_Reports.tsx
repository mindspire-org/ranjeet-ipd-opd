...
export default function Pharmacy_Reports() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [sales, cash, credit, inv, pur, exp, settings] =
          await Promise.all([
            ...
            pharmacyApi
              .getSettings()
              .catch(() => ({ pharmacyName: "Pharmacy" })),
          ]);
        if (!mounted) return;
        setPharmacyName((settings as any)?.pharmacyName || "Pharmacy");
        const [sales2, cash2, credit2, inv2, pur2, exp2] = [
          sales,
          cash,
          credit,
          inv,
        ...
        const monthSales: any = await pharmacyApi.salesSummary({
          from: fmt(startMonth),
          to,
        });
        ...
        const totalProfit = Number(
          (totalSales - totalPurchases - totalExpenses).toFixed(2),
        );
        const monthlyProfit = Number(
          Number(monthSales?.totalProfit || 0).toFixed(2),
        );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
        <SummaryCard
          title="Total Sales"
          value={`PKR ${summary.totalSales.toLocaleString(undefined, { maximumFractionDigits: 3 })}`}
          bg="bg-emerald-50"
        ...
        <SummaryCard
          title="Total Profit"
          value={`PKR ${summary.totalProfit.toLocaleString(undefined, { maximumFractionDigits: 3 })}`}
          bg="bg-indigo-50"
          border="border-indigo-200"
        />
        <SummaryCard
          title="Monthly Profit (Gross Margin)"
          value={`PKR ${summary.monthlyProfit.toLocaleString()}`}
          bg="bg-amber-50"
          border="border-amber-200"
        />
        <SummaryCard
          title="Cash Sales"
          value={`PKR ${summary.cashSales.toLocaleString()}`}
          bg="bg-green-50"
          border="border-green-200"
        />
        <SummaryCard
          title="This Month's Sales"
          value={`PKR ${summary.thisMonthSales.toLocaleString(undefined, { maximumFractionDigits: 3 })}`}
          bg="bg-fuchsia-50"
          border="border-fuchsia-200"
        />
        <SummaryCard
          title="Credit Sales"
          value={`PKR ${summary.creditSales.toLocaleString()}`}
          bg="bg-yellow-50"
          border="border-yellow-200"
        />
        <SummaryCard
          title="Total Inventory"
          value={`PKR ${summary.totalInventory}`}
          bg="bg-cyan-50"
          border="border-cyan-200"
        />
        <SummaryCard
          title="Low Stock Items"
          value={`PKR ${summary.lowStock}`}
          bg="bg-yellow-50"
          border="border-yellow-200"
        />
        <SummaryCard
          title="Out of Stock Items"
          value={`PKR ${summary.outOfStock}`}
          bg="bg-rose-50"
          border="border-rose-200"
        />
        <SummaryCard
          title="Total Stock Value"
          value={`PKR ${summary.stockValue.toLocaleString()}`}
          bg="bg-teal-50"
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-xl border border-slate-200 bg-white p-4 ">
          ...
          <div className="h-56 w-full overflow-hidden rounded-xl border border-slate-100 bg-linear-to-br from-slate-50 to-white p-3 shadow-sm">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          <div className="mb-3 font-medium text-slate-800">
            Comparison: Sales, Purchases, Expenses
          </div>
          <div className="h-56 w-full overflow-hidden rounded-xl border border-slate-100 bg-linear-to-br from-slate-50 to-white p-3 shadow-sm">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
      ...
      <div className="rounded-2xl border-2 border-gradient-to-r from-sky-200 to-indigo-200 bg-linear-to-br from-white to-sky-50/30 p-6 shadow-lg">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-2xl font-black bg-gradient-to-r from-sky-600 to-indigo-600 bg-clip-text text-transparent">
                  Daily Closing Sheet
                </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              onClick={() => {
                ...
                downloadTextFile(
                  `closing-sheet-${pharmacyName.replace(/\s+/g, "-")}-${from}-to-${to}.txt`,
                  text,
                );
              }}
        ...
        <div className="mt-6 rounded-2xl border-2 border-white bg-white/60 backdrop-blur-sm p-6 shadow-inner">
            ...
            <div className="rounded-xl bg-gradient-to-r from-emerald-50 to-teal-50 p-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-bold text-emerald-700">
                  PKR {summary.totalSales.toLocaleString()}
                </span>
              </div>
            </div>
            ...
            <div className="rounded-xl bg-gradient-to-r from-blue-50 to-sky-50 p-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-bold text-blue-700 ">
                  PKR {summary.totalPurchases.toLocaleString()}
                </span>
              </div>
            </div>

            <div className="rounded-xl bg-gradient-to-r from-rose-50 to-pink-50 p-4">
              <div className="flex items-center justify-between">
                <span className="font-semibold text-slate-700">
                  💸 Total Expenses
                </span>
            ...
            <div className="mt-4 rounded-xl bg-slate-50 p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between rounded-lg bg-white p-3">
                  ...
                  <span className="font-bold text-slate-900 ">
                    PKR {summary.creditSales.toLocaleString()}
                  </span>
                </div>
                <div className="flex items-center justify-between rounded-lg bg-white p-3">
                  <span className="text-sm font-medium text-slate-600 ">
                  ...
                  <span className="font-bold text-slate-900 ">
                    PKR {summary.stockValue.toLocaleString()}
                  </span>
                </div>
              </div>
            </div>
