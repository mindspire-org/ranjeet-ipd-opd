...
import Pharmacy_AddInvoice from '../../components/pharmacy/pharmacy_AddInvoice'
...
export default function Pharmacy_Inventory() {
    ...
    const load = async () => {
      try {
        ...
        if (activeTab === 'Pending Review') {
          const res: any = await pharmacyApi.listPurchaseDrafts({ search: search || undefined, limit: 200 })
          ...
          const res: any = await pharmacyApi.listInventory({ search: search || undefined, page, limit })
          ...
          const BIG_LIMIT = 2000
          const res: any = await pharmacyApi.listInventory({ search: search || undefined, page: 1, limit: BIG_LIMIT })
          const items: any[] = res?.items ?? res ?? []
  ...
  return (
    <div className="space-y-6">
      ...
      <Pharmacy_AddInvoice open={addInvoiceOpen} onClose={() => setAddInvoiceOpen(false)} />
