...
import Pharmacy_AddCustomer, {
  type Customer,
} from "../../components/pharmacy/pharmacy_AddCustomer";
...
export default function Pharmacy_Customers() {
  ...
  const exportCsv = async () => {
    try {
      ...
      const first: any = await pharmacyApi.listCustomers({
        q: q || undefined,
        page: 1,
        limit: pageLimit,
      });
      ...
      try {
        ...
        showNotice("error", "Failed to export customers");
      }
    } finally {
      setExporting(false);
    }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res: any = await pharmacyApi.listCustomers({
          q: q || undefined,
          page,
          limit,
        });
    ...
    const handler = () => load();
    try {
      window.addEventListener("pharmacy:sale", handler as any);
    } catch {}
    return () => {
      mounted = false;
      try {
        window.removeEventListener("pharmacy:sale", handler as any);
      } catch {}
    };
  }, [q, page, limit]);
        ...
        <div className="flex items-center gap-3 border-b border-slate-200 px-4 py-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            <button
              ...
              onClick={exportCsv}
              disabled={exporting}
              className="btn-outline-navy disabled:opacity-60"
            >
