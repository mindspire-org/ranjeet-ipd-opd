...
export default function Pharmacy_POSReceiptDialog({ open, onClose, receiptNo, method, lines, discountPct = 0, customer, fbr, autoPrint }: Props) {
  if (!open) return null
        ...
        <div className="w-full max-w-2xl overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black/5 print:shadow-none print:ring-0 print:rounded-none print:max-w-none">
          ...
          <div className="max-h-[75vh] overflow-y-auto px-6 py-6 print:p-0 print:overflow-visible">
            <div id="pharmacy-receipt" className="mx-auto w-[384px] print:w-[384px]">
              ...
              <hr className="my-3 border-dashed" />
              {/* FBR Invoice with QR Code */}
              {fbr?.fbrInvoiceNo ? (
                <div className="mt-2 text-center text-xs text-slate-700 print:text-black">
                  <div><span className="font-semibold">FBR Invoice:</span> {fbr.fbrInvoiceNo} ({fbr.status || 'SUCCESS'})</div>
                  {/* Generate QR Code with all FBR required fields */}
                  <div className="mt-2 flex flex-col items-center gap-1">
                    <QRCodeSVG
                      value={fbr.fbrInvoiceNo}
                      size={96}
                      level="M"
                      includeMargin={false}
                    />
                    <div className="text-[11px] text-slate-600 print:text-black">Scan to verify FBR invoice</div>
                  </div>
                  {String(fbr.mode || '').toUpperCase() === 'SANDBOX' ? (
                    <div className="mt-1 text-[11px] text-amber-700 print:text-black">⚠ Sandbox mode</div>
                  ) : null}
                  {String(fbr.status || '').toUpperCase() === 'FAILED' ? (
                    <div className="mt-1 text-[11px] font-semibold text-rose-600 print:text-black">FBR invoice submission failed</div>
                  ) : null}
                </div>
              ) : null}

              <div className="mt-2 text-center text-xs text-slate-600 print:text-black">{info.footer || 'Thank you for your purchase!'}</div>
            </div>
          </div>
        </div>
      </div>
