...
export default function InvoicePage(_props: { patientId?: string; embedded?: boolean }) {
  ...
  const [fbr, setFbr] = useState<{ fbrInvoiceNo?: string; status?: string; mode?: string; qrDataUrl?: string } | null>(null);
  ...
  function printView() {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8">${style}</head><body>
      ...
      <div class="box">
        ...
        ${fbr?.fbrInvoiceNo ? `
          <div style="margin-top:16px; border-top:2px solid #111; padding-top:10px; display:flex; justify-content:space-between; align-items:flex-start;">
            <div>
              <div style="font-weight:700; font-size:12px; text-transform:uppercase; color:#111; margin-bottom:4px;">FBR Fiscalization</div>
              <div style="font-size:11px;"><b>Invoice No:</b> ${escapeHtml(fbr.fbrInvoiceNo)}</div>
              <div style="font-size:11px;"><b>Status:</b> ${escapeHtml(fbr.status || 'SUCCESS')}</div>
              ${(String(fbr?.mode || '').toUpperCase() === 'SANDBOX' || String(fbr?.mode || '').toUpperCase() === 'MOCK' || !fbr?.mode) ? `<div style="font-size:10px; color:#b45309; margin-top:2px; font-weight:600;">⚠ MOCK FBR – FOR TESTING PURPOSE ONLY</div>` : ''}
              ${String(fbr?.status || '').toUpperCase() === 'FAILED' ? `<div style="font-size:10px; color:#dc2626; margin-top:2px; font-weight:700;">⚠ FBR SYNC FAILED</div>` : ''}
            </div>
            ${fbr.qrDataUrl ? `<div style="text-align:right"><img src="${fbr.qrDataUrl}" alt="FBR QR" style="height:80px;width:80px;object-fit:contain;border:1px solid #111;padding:4px;border-radius:4px;background:#fff"/></div>` : ''}
          </div>
        ` : ''}
      </div>
    </body></html>`;
  ...
  return (
    <div className="bg-white/90 border border-sky-100/70 rounded-xl p-3 md:p-4 shadow-sm">
      ...
      {fbr?.fbrInvoiceNo && (
        <div className="mt-3 bg-sky-50/50 border border-sky-100 rounded-lg p-3 flex items-center justify-between">
          <div>
            <div className="text-xs font-semibold text-sky-800 uppercase tracking-wider">FBR Fiscalization</div>
            <div className="text-sm font-medium text-slate-700">Inv: {fbr.fbrInvoiceNo}</div>
            <div className="text-[10px] text-slate-500 uppercase tracking-tighter">Status: {fbr.status} | Mode: {fbr.mode || 'Production'}</div>
          </div>
          {fbr.qrDataUrl && (
            <img src={fbr.qrDataUrl} className="h-14 w-14 object-contain border border-sky-200 rounded p-0.5 bg-white" alt="FBR QR" />
          )}
        </div>
      )}
