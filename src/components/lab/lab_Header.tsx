    ...
    <header className="sticky top-0 z-10 h-16 w-full border-b border-slate-200 bg-white/90 backdrop-blur dark:border-slate-700 dark:bg-slate-900/80">
      <div className="mx-auto flex h-full max-w-7xl items-center gap-3 px-4 sm:px-6">
        ...
        <div className="ml-auto flex items-center gap-3 text-sm">
          ...
          <button aria-pressed={theme === 'dark'} onClick={(e) => { try { const nextTheme = (theme === 'dark' ? 'light' : 'dark'); const scope = (e.currentTarget as HTMLElement).closest('.lab-scope') as HTMLElement | null; if (scope) { scope.classList.toggle('dark', nextTheme === 'dark') } try { localStorage.setItem('lab.theme', nextTheme) } catch {}; } catch {}; if (onToggleTheme) onToggleTheme() }} className="hidden rounded-md border border-slate-200 px-3 py-1.5 text-slate-700 hover:bg-slate-50 sm:flex items-center gap-2 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
