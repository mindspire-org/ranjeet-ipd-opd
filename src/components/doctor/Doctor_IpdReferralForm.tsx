...
import Hospital_Modal from "../hospital/bed-management/Hospital_Modal";
...
export default forwardRef(function Doctor_IpdReferralForm(
  { mrn, doctor, onSaved }: Props,
  ref: any,
) {
  ...
  async function loadPatient(m: string) {
    setLoading(true);
    try {
      const res = (await hospitalApi.searchPatients({
        mrn: m,
        limit: 1,
      })) as any;
      ...
      try {
        if (m) {
          ...
          if (lp) {
            const merged: any = { ...p };
            if (!merged?.cnic && (lp.cnic || lp.cnicNormalized))
              merged.cnic = lp.cnic || lp.cnicNormalized;
            if (!merged?.address && lp.address) merged.address = lp.address;
            if (!merged?.phone && (lp.phone || lp.phoneNormalized))
              merged.phone = lp.phone || lp.phoneNormalized;
            if (!merged?.fatherName && lp.fatherName)
              merged.fatherName = lp.fatherName;
  ...
  const canSave = useMemo(
    () => !!patient?._id && !!form.reasonOfReferral.trim(),
    [patient?._id, form.reasonOfReferral],
  );
  ...
  const patientAge = useMemo(() => {
    ...
    if (p.dob) {
      try {
        ...
        if (!isNaN(d as any)) {
          const y = Math.floor((Date.now() - d.getTime()) / 31557600000);
  ...
  useImperativeHandle(
    ...
    () => ({
      getPreviewData: () => {
        const depRow = deps.find(
          (d) => String(d._id || d.id) === form.referredTo.departmentId,
        );
        const docRow = docs.find(
          (d) => String(d._id || d.id) === form.referredTo.doctorId,
        );
        const p = patient;
        const patientObj = {
  ...
  async function save() {
    if (!patient?._id) {
      showModal("Error", "Patient not found", "error");
      return;
    }
    setSaving(true);
    try {
      const isHex24 = (s?: string) => !!s && /^[a-f\d]{24}$/i.test(s);
      const depId = isHex24(form.referredTo.departmentId)
        ? form.referredTo.departmentId
        : undefined;
      const docId = isHex24(form.referredTo.doctorId)
        ? form.referredTo.doctorId
        : undefined;
      const payload = {
      ...
      onSaved?.(id);
      showModal("Success", "Referral saved", "success");
    } catch (e: any) {
      try {
        ...
        const item = {
          ...
          referredTo: {
            departmentId: form.referredTo.departmentId || "",
            departmentName: depRow?.name,
            doctorId: form.referredTo.doctorId || "",
            doctorName: docRow?.name,
          },
          referralDate: form.referralDate,
          referralTime: form.referralTime,
          reasonOfReferral: form.reasonOfReferral,
          provisionalDiagnosis: form.provisionalDiagnosis,
          vitals: { ...form.vitals },
          condition: {
            stability: form.stability,
        ...
        showModal(
          "Warning",
          `Failed to save on server: ${e?.message || "Unknown error"}. Referral saved locally.`,
          "error",
        );
      } catch (e: any) {
        showModal("Error", e?.message || "Failed to save", "error");
      }
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div className="rounded-md border border-slate-200 p-3 bg-slate-50">
          ...
          <div className="grid gap-2 text-sm">
            ...
            <div>
              ...
              <span>{patient?.mrn || patient?.mrNumber || mrn || "-"}</span>
            </div>
            <div>
              <span className="text-slate-500">Age:</span>{" "}
              <span>{patientAge || "-"}</span>{" "}
              <span className="ml-3 text-slate-500">Gender:</span>{" "}
              <span className="capitalize">{patient?.gender || "-"}</span>
            </div>
            <div>
              <span className="text-slate-500">Father/Husband:</span>{" "}
              <span className="capitalize">{patient?.fatherName || "-"}</span>
            </div>
            <div>
              <span className="text-slate-500">CNIC:</span>{" "}
              <span>
                {patient?.cnic || (patient as any)?.cnicNormalized || "-"}
              </span>
            </div>
            <div>
              <span className="text-slate-500">Phone:</span>{" "}
              <span>{patient?.phone || patient?.phoneNormalized || "-"}</span>
            </div>
            <div>
        ...
        <div className="grid gap-3">
          <label className="block text-sm">
            <span className="text-slate-600 block mb-1">Date of Referral</span>
            <input
              type="date"
              value={form.referralDate}
              onChange={(e) =>
                setForm((f) => ({ ...f, referralDate: e.target.value }))
              }
              className="w-full rounded-md border border-slate-300 px-3 py-2"
            />
          </label>
          <label className="block text-sm">
            <span className="text-slate-600 block mb-1">Time of Referral</span>
            <input
              type="time"
              value={form.referralTime}
              onChange={(e) =>
                setForm((f) => ({ ...f, referralTime: e.target.value }))
              }
              className="w-full rounded-md border border-slate-300 px-3 py-2"
            />
          </label>
          <label className="block text-sm">
            ...
            <input
              value={form.reasonOfReferral}
              onChange={(e) =>
                setForm((f) => ({ ...f, reasonOfReferral: e.target.value }))
              }
              className="w-full rounded-md border border-slate-300 px-3 py-2"
            />
          </label>
          <label className="block text-sm">
            ...
            <input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, provisionalDiagnosis: e.target.value }))
              }
              className="w-full rounded-md border border-slate-300 px-3 py-2"
            />
          </label>
      ...
      <div className="grid gap-3 md:grid-cols-3 mt-3">
        <label className="block text-sm">
          <span className="text-slate-600 block mb-1">BP</span>
          <input
            value={form.vitals.bp}
            onChange={(e) =>
              setForm((f) => ({
                ...f,
                vitals: { ...f.vitals, bp: e.target.value },
              }))
            }
            className="w-full rounded-md border border-slate-300 px-3 py-2"
            placeholder="120/80"
        ...
        <label className="block text-sm">
          <span className="text-slate-600 block mb-1">Pulse</span>
          <input
            value={form.vitals.pulse}
            onChange={(e) =>
              setForm((f) => ({
                ...f,
                vitals: { ...f.vitals, pulse: e.target.value },
              }))
            }
            className="w-full rounded-md border border-slate-300 px-3 py-2"
          />
        ...
        <label className="block text-sm">
          <span className="text-slate-600 block mb-1">Temperature</span>
          <input
        ...
        <label className="block text-sm">
          <span className="text-slate-600 block mb-1">RR</span>
          <input
            value={form.vitals.rr}
            onChange={(e) =>
              setForm((f) => ({
                ...f,
                vitals: { ...f.vitals, rr: e.target.value },
              }))
            }
            className="w-full rounded-md border border-slate-300 px-3 py-2"
          />
      ...
      <div className="grid gap-3 md:grid-cols-2 mt-3">
        <div className="grid gap-3">
          <label className="block text-sm">
            ...
            <select
              value={form.referredTo.departmentId}
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  referredTo: { ...f.referredTo, departmentId: e.target.value },
                }))
              }
              className="w-full rounded-md border border-slate-300 px-3 py-2"
            >
          ...
          <label className="block text-sm">
            <span className="text-slate-600 block mb-1">Doctor (optional)</span>
            <select
              value={form.referredTo.doctorId}
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  referredTo: { ...f.referredTo, doctorId: e.target.value },
                }))
              }
              className="w-full rounded-md border border-slate-300 px-3 py-2"
            >
        ...
        <div className="grid gap-3">
          <div className="grid grid-cols-2 gap-2">
            <label className="block text-sm">
              <span className="text-slate-600 block mb-1">Condition</span>
              <select
                value={form.stability}
                onChange={(e) =>
                  setForm((f) => ({ ...f, stability: e.target.value as any }))
                }
                className="w-full rounded-md border border-slate-300 px-3 py-2"
              >
            ...
            <label className="block text-sm">
              <span className="text-slate-600 block mb-1">Consciousness</span>
              <select
          ...
          <label className="block text-sm">
            <span className="text-slate-600 block mb-1">Any other Remarks</span>
            <textarea
              rows={3}
              value={form.remarks}
              onChange={(e) =>
          ...
          <label className="block text-sm">
            <span className="text-slate-600 block mb-1">
              Sign & Stamp of Doctor
            </span>
            <input
              value={form.signStamp}
              onChange={(e) =>
                setForm((f) => ({ ...f, signStamp: e.target.value }))
              }
              className="w-full rounded-md border border-slate-300 px-3 py-2"
            />
          </label>
      ...
      {loading && (
        <div className="mt-2 text-sm text-slate-500">Loading patient…</div>
      )}
      {!patient && !loading && mrn && (
        <div className="mt-2 text-sm text-rose-600">
          No patient found for MRN: {mrn}
        </div>
      )}
      {modal.open && (
        <Hospital_Modal
